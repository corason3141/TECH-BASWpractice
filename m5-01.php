...
<body>
 ...
 <?php
    ...
    ."comment TEXT,"
    ."date char(32),"
    ."pass char(32)"//ここに「,」が入っててエラーが起きてた！
    .");";
    $stmt = $pdo->query($sql);
...
<?php
      ...
      //編集実行機能
if(!empty($_POST["edit-number"])){//editNoはいらないかも！
    $id = $_POST["edit-number"];      //ここで編集対象番号の値の受け取りを行う
    $name = $_POST["name"];
    $comment = $_POST["comment"];
    $password = $_POST["password"]; 
    $date = date("Y年m月d日 H時i分s秒");
    $sql = 'update mission5_1 set name=:name,comment=:comment,pass=:pass,date=:date where id=:id';//dateのところに「:」が足りなかった！
